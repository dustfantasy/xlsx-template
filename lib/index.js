    ...
    */
    ...
    Workbook.prototype.substitute = function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(function(row, rowIndex) {
            row.attrib.r = currentRowNumber = self.getCurrentRow(row, totalRowsInserted);
            rows.push(row);
            ...
            row.findall("c").forEach(function(cell) {
                var appendCell = true;
                var originalCell = self.cloneElement(cell)

                cell.attrib.r = self.getCurrentCell(cell, currentRowNumber, cellsInserted);
                ...
                if(cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function(placeholder) {
                        ...
                        if(placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                            currentRow = row
                            if (placeholderRows[placeholder.name]) {
                                currentRow = self.cloneElement(placeholderRows[placeholder.name][0])
                                startRowNumber = currentRow.attrib.r
                                currentRow.attrib.r = currentRowNumber = startRowNumber + rowIndex - placeholderRows[placeholder.name][1]
                                rows[rows.length - 1].attrib.r = currentRow.attrib.r
                                // 最新的row以template的當前rows位置與插入數量算出最後的位置
                                // 但因為gap功能row位置必須重新指定
                                cell.attrib.r = self.getCurrentCell(originalCell, currentRowNumber, cellsInserted);
                            }
			
				            gap = substitution[0] && substitution[0].gap || 1
                            if (gap > 1 && !placeholderRows[placeholder.name]) {
                                placeholderRows[placeholder.name] = [row, rowIndex]
                            }    

				            newCellsInserted = self.substituteTable(
                                currentRow, newTableRows,
                                cells, cell,
                ...
                var lastCellCoords
                if (cells.length > 0) {
                    // FIXME Prevent double insertion when combination of table+array
                    lastCellCoords = cells[cells.length - 1].attrib.r
                }
                if(appendCell && (cell.attrib.r !== lastCellCoords)) {
                    cells.push(cell);
                }
            ...
            if(newTableRows.length > 0) {
                ...
                self.pushDown(self.workbook, sheet.root, namedTables, currentRowNumber, newTableRows.length, gap, startRowNumber);
            }
        ...
        // 由於原先替換的cell只會替換被賦予值的位置， 這導致其餘cell沒有樣式，因此用前一格的內容進行填充
        sheetData._children.forEach(row => {
            let lastC
            row._children.forEach(col => {
                while(lastC && self.nextCol(lastC.attrib.r) !== col.attrib.r) {
                    var newCell = self.cloneElement(lastC);
                    newCell.attrib.r = self.nextCol(lastC.attrib.r);
                    row._children.push(newCell)
                    lastC = newCell
                }
                lastC = col
            })
            row._children.sort((a,b) => a.attrib.r.localeCompare(b.attrib.r))
        })

        sheetData._children.sort((a,b) => a.attrib.r - b.attrib.r)
        // 若排序不一，windows excel會出現格式錯誤
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    if((idx - 1) < newTableRows.length) {
                        ...
                        newRow.attrib.r = self.getCurrentRow(row, (newTableRows.length+1)*(element.gap||1));
    ...
    Workbook.prototype.pushDown = function(workbook, sheet, tables, currentRowNumber, numRows, gap, startRowNumber) {
        var self = this;
	...
	var mergeCells = sheet.find("mergeCells");
        ...
        sheet.findall("mergeCells/mergeCell").forEach(function(mergeCell) {
            var mergeRange    = self.splitRange(mergeCell.attrib.ref),
                mergeStart    = self.splitRef(mergeRange.start),
                mergeEnd      = self.splitRef(mergeRange.end);
		    var isStartRow = currentRowNumber === startRowNumber

            if (startRowNumber && !isStartRow) return
            if(mergeStart.row >= currentRowNumber + gap) {
                mergeStart.row += numRows * gap;
                mergeEnd.row += numRows * gap;
		...
		//add new merge cell
            if (mergeStart.row >= currentRowNumber && mergeStart.row < currentRowNumber + gap) {
              for (var i = 1; i <= numRows; i++) {
                var newMergeCell = self.cloneElement(mergeCell);
                mergeStart.row += 1 * gap;
                mergeEnd.row += 1 * gap;
        ...
        tables.forEach(function(table) {
            ...
            if(tableStart.row > currentRowNumber) {
        ...
        workbook.findall("definedNames/definedName").forEach(function(name) {
            ...
            if(self.isRange(ref)) {
                ...
                if(namedStart){
                    if(namedStart.row > currentRowNumber) {
                ...
                if(namedRef.row > currentRowNumber) {
